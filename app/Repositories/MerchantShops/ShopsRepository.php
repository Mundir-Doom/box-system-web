...
class ShopsRepository implements ShopsInterface{
    ...
    public function storeWithDefaults($request){
        try {
            // If this shop should be the default, unset other default shops
            if($request->default_shop) {
                MerchantShops::where('merchant_id', $request->merchant_id)
                    ->update(['default_shop' => 0]);
            }
            ...
            $shop->contact_no       = $request->contact_no;
            $shop->address          = $request->address;
            $shop->merchant_lat     = $request->lat ?? null;
            $shop->merchant_long    = $request->long ?? null;
            $shop->status           = $request->status ?? 1;
            $shop->default_shop     = $request->default_shop ? 1 : 0;
            $shop->save();
            return true;
