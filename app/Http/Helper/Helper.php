...
if(!function_exists('hasPermission')){
    function hasPermission($permission=null){
        ...
        if($user->role && in_array($permission, $user->role->permissions ?? [])){
            return true;
        }
...
if(!function_exists('settings')){
    function settings(){
        ...
        try {
            $dbSettings = App\Models\Backend\GeneralSettings::with('rxlogo','rxfavicon')->find(1);
        } catch (\Throwable $e) {
            $dbSettings = null;
        }

        if ($dbSettings) {
            return $cachedSettings = $dbSettings;
        }

        // Fallback object to prevent null errors when settings are not seeded yet
        $cachedSettings = (object) [
            ...
            'pound_rate'      => 1,
            // Commonly used derived attributes with safe defaults
            'logo_image'      => static_asset('images/default/logo.png'),
            'light_logo_image'=> static_asset('images/default/light-logo.png'),
            'favicon_image'   => static_asset('images/default/favicon.png'),
            // Paths used by exporters
            'PLogoImage'      => public_path('images/default/logo.png'),
            // Nested relation-like object used in calculations
            'excenseRate'     => (object) ['exchange_rate' => 1],
            // Misc
            'copyright'       => '',
            'primary_color'   => '#d60b18',
            'text_color'      => '#212529',
        ];
...
if(!function_exists('setEnv')){
    function setEnv($name, $value){
        // For safety, only allow runtime .env changes in local/testing
...
if(!function_exists('dateFormat')){
    function dateFormat($newDate=null){
        try {
            ...
            $dt = \Carbon\Carbon::parse($newDate);
...
if(!function_exists('parcelStatus')){
    function parcelStatus($parcel,$request=null){
        ...
        }elseif($parcel->status == ParcelStatus::RECEIVED_BY_PICKUP_MAN){
            $allowStatus = [
        ...
        elseif($parcel->status == ParcelStatus::RETURN_WAREHOUSE){
            // From Return Warehouse, allow assigning return to merchant flow
            $allowStatus = [ParcelStatus::RETURN_ASSIGN_TO_MERCHANT];
        }
        ...
        elseif($parcel->status == ParcelStatus::RETURNED_MERCHANT){
        }
            ...
            foreach($parcelStatusArray as $key => $status) {
                if($key == ParcelStatus::PICKUP_ASSIGN_CANCEL){
                    ...
                    if($key == ParcelStatus::PICKUP_RE_SCHEDULE){
                        $parcelStatus .= '<a  class="dropdown-item parcel-id-pickup-man" data-parcelstatus="'.ParcelStatus::PICKUP_ASSIGN.'" data-parcel="'.$parcel->id.'" data-bs-toggle="modal" data-bs-target="#parcelstatus'.$key.'" href="#">'.$status.'</a>';
                    }elseif($key == ParcelStatus::TRANSFER_TO_HUB){
                        $parcelStatus .= '<a  class="dropdown-item parcel-id-transfer-hub" data-parcel="'.$parcel->id.'" data-bs-toggle="modal" data-bs-target="#parcelstatus'.$key.'" href="#">'.$status.'</a>';
                    }elseif($key == ParcelStatus::DELIVERY_RE_SCHEDULE){
                        $parcelStatus .= '<a  class="dropdown-item parcel-id-delivery-man" data-parcelstatus="'.ParcelStatus::DELIVERY_MAN_ASSIGN.'" data-parcel="'.$parcel->id.'" data-bs-toggle="modal" data-bs-target="#parcelstatus'.$key.'" href="#">'.$status.'</a>';
                    }elseif($key == ParcelStatus::RECEIVED_WAREHOUSE){

                        $parcelStatus .= '<a  class="dropdown-item parcel-id received_warehouse" data-parcel="'.$parcel->id.'" data-bs-toggle="modal" data-parcel="'. $parcel->id.'"  data-hub="'.$parcel->hub_id.'" data-url="'.route('parcel.received.warehouse.hub.select').'" data-bs-target="#parcelstatus'.$key.'" href="#">'.$status.'</a>';
                    }
                    else{

                        $parcelStatus .= '<a  class="dropdown-item parcel-id" data-parcel="'.$parcel->id.'" data-bs-toggle="modal" data-bs-target="#parcelstatus'.$key.'" href="#">'.$status.'</a>';
...
if (!function_exists('totalParcelsCashcollection')) {
    function totalParcelsCashcollection($parcels)
   {
        ...
        foreach ($parcels as $parcel) {
            if ($parcel instanceof \Illuminate\Support\Collection) {
                $total_cash_collection += (float) $parcel->sum('cash_collection');
            } else {
                $total_cash_collection += (float) ($parcel->cash_collection ?? 0);
            }
        }
...
if (!function_exists('withoutUser')) {
    ...
    function withoutUser($ids)
    {
        ...
        if (!blank($ids)) {
            $query->whereNotIn('id', (array)$ids);
        }

        $user = $query->get();
        return !blank($user) ? $user : [];
    }
}
...
if (!function_exists('unpaidUser')) {
    function unpaidUser($ids)
    {
        ...
        $users = User::whereIn('id',(array)$ids)
            ->whereNotIn('user_type',[UserType::DELIVERYMAN,UserType::MERCHANT])
            ->where('status',Status::ACTIVE)
            ->get();
        return !blank($users) ? $users : [];
    }
}


// Avoid colliding with the zero-argument user() helper above
...
if (!function_exists('singleUser')) {
            ...
            if(!blank($todoStatusArray)){
                foreach($todoStatusArray as $key => $status) {
                    if($key == todoStatus::PENDING){
                        $todoStatus .= '<a  class="dropdown-item pending" data-title="pending" data-id="'.$todo->id.'" id="todo_btn" data-url="'.route("todo.processing").'" data-bs-toggle="modal" data-bs-target="#todoStatus'.$key.'"  href="#">'.$status.'</a>';
                    }elseif($key == todoStatus::PROCESSING){
                        $todoStatus .= '<a  class="dropdown-item processing" data-id="'.$todo->id.'" id="todo_btn" data-title="processing" data-url="'.route("todo.processing").'" data-bs-toggle="modal" data-bs-target="#todoStatus'.$key.'"  href="#">'.$status.'</a>';
                    }
                    else{
                        $todoStatus .= '<a  class="dropdown-item completed" data-title="completed" data-id="'.$todo->id.'" id="todo_btn" data-url="'.route("todo.completed").'" data-bs-toggle="modal" data-bs-target="#todoStatus1'.$key.'"  href="#">'.$status.'</a>';
                    }
                }
                return $todoStatus;
            }
    ...
    // Scoped merchant income by merchant_id
    if (!function_exists('dayMerchantIncomeById')) {
        function dayMerchantIncomeById($date, $merchant_id)
        {
            $date = Carbon::parse($date)->format('Y-m-d');
            $q = MerchantStatement::where('type', AccountHeads::INCOME)
                ->whereDate('date', $date);
    ...
    // Scoped merchant expense by merchant_id
    if (!function_exists('dayMerchantExpenseById')) {
        function dayMerchantExpenseById($date, $merchant_id)
        {
            $date = Carbon::parse($date)->format('Y-m-d');
            $q = MerchantStatement::where('type', AccountHeads::EXPENSE)
                ->whereDate('date', $date);
            if (!empty($merchant_id)) {
                $q->where('merchant_id', $merchant_id);
            }
    ...
    // Scoped deliveryman income by delivery_man_id
    if (!function_exists('dayDeliverymanIncomeById')) {
        function dayDeliverymanIncomeById($date, $delivery_man_id)
        {
            $date = Carbon::parse($date)->format('Y-m-d');
            $q = DeliverymanStatement::where('type', AccountHeads::INCOME)
                ->whereDate('date', $date);
    ...
    // Scoped deliveryman expense by delivery_man_id
    if (!function_exists('dayDeliverymanExpenseById')) {
        function dayDeliverymanExpenseById($date, $delivery_man_id)
        {
            $date = Carbon::parse($date)->format('Y-m-d');
            $q = DeliverymanStatement::where('type', AccountHeads::EXPENSE)
                ->whereDate('date', $date);
            if (!empty($delivery_man_id)) {
                $q->where('delivery_man_id', $delivery_man_id);
            }
    ...
    // Scoped hub income by hub_id
    if (!function_exists('dayHubIncomeById')) {
        function dayHubIncomeById($date, $hub_id)
        {
            $date = Carbon::parse($date)->format('Y-m-d');
            $q = HubStatement::where('type', AccountHeads::INCOME)
                ->whereDate('created_at', $date);
            if (!empty($hub_id)) {
                $q->where('hub_id', $hub_id);
            }
            return (float) $q->sum('amount');
        }
    }

    // Scoped hub expense by hub_id
    if (!function_exists('dayHubExpenseById')) {
        function dayHubExpenseById($date, $hub_id)
        {
            $date = Carbon::parse($date)->format('Y-m-d');
            $q = HubStatement::where('type', AccountHeads::EXPENSE)
                ->whereDate('created_at', $date);
            if (!empty($hub_id)) {
                $q->where('hub_id', $hub_id);
            }
