...
use App\Http\Controllers\Backend\CollectionPeriodsController;
use App\Http\Controllers\Backend\CollectionSessionsController;
use App\Http\Controllers\Backend\DeliveryAssignmentController;
    ...
    Route::group(['middleware' => 'auth'], function () {
        ...
        Route::group(['middleware' => 'XSS'], function () {
            ...
            Route::group(['prefix' => 'admin'], function () {
                ...
                Route::post('merchant/shops/store-ajax',    [MerchantShopsController::class, 'storeAjax'])->name('merchant.shops.store.ajax')->middleware('hasPermission:merchant_shop_create');
                ...
                Route::group(['prefix' => 'collection-periods'], function () {
                    Route::get('/', [CollectionPeriodsController::class, 'index'])->name('collection-periods.index')->middleware('hasPermission:collection_periods_read');
                    Route::get('/create', [CollectionPeriodsController::class, 'create'])->name('collection-periods.create')->middleware('hasPermission:collection_periods_create');
                    Route::post('/', [CollectionPeriodsController::class, 'store'])->name('collection-periods.store')->middleware('hasPermission:collection_periods_create');
                    Route::get('/{id}/edit', [CollectionPeriodsController::class, 'edit'])->name('collection-periods.edit')->middleware('hasPermission:collection_periods_update');
                    Route::put('/{id}', [CollectionPeriodsController::class, 'update'])->name('collection-periods.update')->middleware('hasPermission:collection_periods_update');
                    Route::delete('/{id}', [CollectionPeriodsController::class, 'destroy'])->name('collection-periods.destroy')->middleware('hasPermission:collection_periods_delete');
                    Route::post('/{id}/toggle', [CollectionPeriodsController::class, 'toggle'])->name('collection-periods.toggle')->middleware('hasPermission:collection_periods_update');
                    Route::get('/current-active', [CollectionPeriodsController::class, 'getCurrentActive'])->name('collection-periods.current-active');
                });

                Route::group(['prefix' => 'collection-sessions'], function () {
                    Route::get('/', [CollectionSessionsController::class, 'index'])->name('collection-sessions.index')->middleware('hasPermission:collection_sessions_read');
                    Route::get('/current', [CollectionSessionsController::class, 'current'])->name('collection-sessions.current')->middleware('hasPermission:collection_sessions_read');
                    Route::get('/history', [CollectionSessionsController::class, 'history'])->name('collection-sessions.history')->middleware('hasPermission:collection_sessions_read');
                    Route::get('/{id}', [CollectionSessionsController::class, 'show'])->name('collection-sessions.show')->middleware('hasPermission:collection_sessions_read');
                    Route::post('/start', [CollectionSessionsController::class, 'start'])->name('collection-sessions.start')->middleware('hasPermission:collection_sessions_create');
                    Route::post('/{id}/complete', [CollectionSessionsController::class, 'complete'])->name('collection-sessions.complete')->middleware('hasPermission:collection_sessions_update');
                    Route::post('/{id}/parcels', [CollectionSessionsController::class, 'addParcel'])->name('collection-sessions.add-parcel')->middleware('hasPermission:collection_sessions_update');
                    Route::delete('/{id}/parcels/{parcelId}', [CollectionSessionsController::class, 'removeParcel'])->name('collection-sessions.remove-parcel')->middleware('hasPermission:collection_sessions_update');
                    Route::post('/{id}/bulk-add-parcels', [CollectionSessionsController::class, 'bulkAddParcels'])->name('collection-sessions.bulk-add-parcels')->middleware('hasPermission:collection_sessions_update');
                    Route::get('/data/sessions', [CollectionSessionsController::class, 'getSessionsData'])->name('collection-sessions.data');
                });

                Route::group(['prefix' => 'delivery-assignments'], function () {
                    Route::get('/', [DeliveryAssignmentController::class, 'index'])->name('delivery-assignments.index')->middleware('hasPermission:delivery_assignments_read');
                    Route::post('/assign', [DeliveryAssignmentController::class, 'assign'])->name('delivery-assignments.assign')->middleware('hasPermission:delivery_assignments_create');
                    Route::get('/available-delivery-men', [DeliveryAssignmentController::class, 'getAvailableDeliveryMen'])->name('delivery-assignments.available-delivery-men');
                });
                Route::post('parcel/delivery-charge',                   [ParcelController::class, 'deliveryCharge'])->name('parcel.deliveryCharge.get');
                
                // Hub Pricing Routes
                Route::get('hub-pricing', [\App\Http\Controllers\Backend\HubPricingController::class, 'index'])->name('hub-pricing.index');
                Route::post('hub-pricing/store', [\App\Http\Controllers\Backend\HubPricingController::class, 'store'])->name('hub-pricing.store');
                Route::delete('hub-pricing/{id}', [\App\Http\Controllers\Backend\HubPricingController::class, 'destroy'])->name('hub-pricing.destroy');
                
                // Invoice Number Generation
                Route::get('generate-invoice-number', [\App\Http\Controllers\Backend\ParcelController::class, 'generateInvoiceNumber'])->name('parcel.generate-invoice-number');
