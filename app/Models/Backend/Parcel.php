...
class Parcel extends Model
{
    ...
    protected $fillable = [
        ...
        'collection_session_id', 'collected_at', 'is_priority_delivery'
    ];
    ...
    */
    ...
    public function getParcelStatusAttribute()
    {
        ...
        elseif($this->status == ParcelStatus::RETURNED_MERCHANT) {
            ...
            $transferHubName = $this->transferhub ? $this->transferhub->name : 'Unknown Transfer Hub';
    ...
    public function getBarcodePrintAttribute()
    {
        if (empty($this->tracking_id)) {
            return '<div style="text-align:center; padding:10px; border:1px dashed #ccc;">No Tracking ID</div>';
        }
    ...
    /**
     * Get the collection assignment
     */
    public function collectionAssignment()
    {
        return $this->hasOne(ParcelCollectionAssignment::class);
    }
    ...
    /**
     * Check if parcel is assigned to delivery
     */
    public function isAssignedToDelivery()
    {
        return $this->collectionAssignment && $this->collectionAssignment->isAssigned();
    }
