    ...
    public function storeAjax(Request $request){
        if($request->ajax()) {
            ...
            $request->validate([
                ...
                'default_shop' => 'nullable|boolean'
            ]);

            if($this->repo->storeWithDefaults($request)){
                return response()->json([
                    'success' => true,
                    'message' => __('merchantshops.added_msg')
                ]);
            }else{
                return response()->json([
                    'success' => false,
                    'message' => __('merchantshops.error_msg')
                ], 500);
            }
        }
