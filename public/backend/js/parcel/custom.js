$(document).ready(function(){
    ...
    $(".parcel-id-transfer-hub").click(function(){
        ...
        $.ajax({
            ...
            success : function (data) {
                ...
                // Auto-select the first available hub to prevent required validation error on first submit
                var firstVal = $('#transfer-hub option:first').val();
                if (firstVal) {
                    $('#transfer-hub').val(firstVal);
                }
    ...
    // Initialize Select2 for generic delivery man selects in modals
    $(".delivery_man_search").each(function() {
        var $el = $(this);
        ...
        $el.select2({
            ...
            ajax: {
                url: $el.data('url') || $("#delivery_man_search").data('url'),
                type: "POST",
                dataType: 'json',
                delay: 250,
                data: function (params) {
    ...
    $( ".delivery_man_search_hub" ).select2({
        // Ensure dropdown renders within the modal and input is focusable
        dropdownParent: $("#parcelstatus6"),
        width: '100%',
    ...
    // Auto-open and focus the Select2 search when modal opens
    $('#parcelstatus6').on('shown.bs.modal', function () {
        // Re-init to ensure proper parent and open the search field
        setTimeout(function() {
            if (!$('#delivery_man_search_hub').hasClass('select2-hidden-accessible')) {
                $("#delivery_man_search_hub").select2({
                    dropdownParent: $("#parcelstatus6"),
                    width: '100%',
                    placeholder: 'Select delivery man',
            ...
            $("#delivery_man_search_hub").select2('open');
        }, 100);
    });
    ...
    //select assign type (Bootstrap 5 safe)
