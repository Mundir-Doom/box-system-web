    ...
    public function parcelStatusUpdate(Request $request){
        ...
        switch ($request->status_action) {
            ...
            case ParcelStatus::DELIVERY_RE_SCHEDULE:
                if($this->parcel->deliveryReschedule($request->parcel_id, $request)):
                    return $this->responseWithSuccess(__('parcel.delivery_reschedule_success'),[],200);
                else:
                    return $this->responseWithError(__('parcel.error_msg'), [],500);
                endif;
                break;

            //delivery man assign (this might be used as "return to courier" in the app)
            ...
            case ParcelStatus::TRANSFER_TO_HUB:
                if($this->parcel->transfertohub($request->parcel_id, $request)):
                    return $this->responseWithSuccess(__('parcel.transfer_to_hub_success'),[],200);
                else:
                    return $this->responseWithError(__('parcel.error_msg'), [],500);
                endif;
                break;
